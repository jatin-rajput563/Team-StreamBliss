...
const Header = () => {
  ...
  return (
    <>
      <nav className='px-3 max-w-[1140px] mx-auto py-4.5 md:py-[21.5px]'>
        <div className="flex justify-between items-center">
          ...
          <ul id='nav-name' className={`list-unstyled flex justify-center items-center nav-link gap-7 lg:gap-[35px] mb-0 ${menuOpen ? "show-navbar" : ""}`}>
            ...
            <li className='lg:hidden'><CustomButton onClick={linkClick} btnClass={"px-[55px] py-[14px]"} btnText={"Dashboard"} /></li>
            <li className='lg:hidden'><a onClick={linkClick} className='border-[#B3B3B4] border h-[57px] w-[57px] rounded-full flex items-center justify-center' href="/"><ProfileIcon /></a></li>
          </ul>
          <ul className='max-lg:hidden flex gap-4 items-center justify-center'>
            <li><CustomButton onClick={linkClick} btnClass={"px-[55px] py-[14px]"} btnText={"Dashboard"} /></li>
            <li><a className='border-[#B3B3B4] border h-[57px] w-[57px] rounded-full flex items-center justify-center hover:bg-gradient-to-b from-[#B851E0] to-[#EB489B] hover:border-transparent transition-all duration-200 ease-linear' href="/"><ProfileIcon /></a></li>
          </ul>
        </div>
      </nav>
